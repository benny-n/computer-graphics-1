...
float quantize(float og) {
	if (og > 0.875) return 1;
	if (og > 0.675) return 0.75;
	if (og > 0.375) return 0.5;
	if (og > 0.2) return 0.25;
	return 0.15;
}
...
void main() {
    ...
    vec4 real_color = reflectivity * reflective_color + (1 - reflectivity) * base_color;
	fColor = vec4(quantize(real_color.r), quantize(real_color.g), quantize(real_color.b), 1);
}
